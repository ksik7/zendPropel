 ...
 * @method BookQuery orderByAutorId($order = Criteria::ASC) Order by the autor_id column
 *
 * @method BookQuery groupById() Group by the id column
 * @method BookQuery groupByName() Group by the name column
 * @method BookQuery groupByAutorId() Group by the autor_id column
 *
 ...
 *
 * @method BookQuery leftJoinAutor($relationAlias = null) Adds a LEFT JOIN clause to the query using the Autor relation
 * @method BookQuery rightJoinAutor($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Autor relation
 * @method BookQuery innerJoinAutor($relationAlias = null) Adds a INNER JOIN clause to the query using the Autor relation
 *
 ...
 * @method Book findOneByAutorId(int $autor_id) Return the first Book filtered by the autor_id column
 *
 ...
 * @method array findByAutorId(int $autor_id) Return Book objects filtered by the autor_id column
 *
...
abstract class BaseBookQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT `id`, `name`, `autor_id` FROM `books` WHERE `id` = :p0';
        try {
     ...
     * $query->filterByAutorId(array('max' => 12)); // WHERE autor_id <= 12
     * </code>
     *
     * @see       filterByAutor()
     *
    ...
    public function filterByAutorId($autorId = null, $comparison = null)
    {
        if (is_array($autorId)) {
            ...
            if (isset($autorId['min'])) {
                $this->addUsingAlias(BookPeer::AUTOR_ID, $autorId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($autorId['max'])) {
                $this->addUsingAlias(BookPeer::AUTOR_ID, $autorId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(BookPeer::AUTOR_ID, $autorId, $comparison);
    }
    ...
    public function filterByAutor($autor, $comparison = null)
    {
        if ($autor instanceof Autor) {
            return $this
                ->addUsingAlias(BookPeer::AUTOR_ID, $autor->getId(), $comparison);
            ...
            return $this
                ->addUsingAlias(BookPeer::AUTOR_ID, $autor->toKeyValue('PrimaryKey', 'Id'), $comparison);
        } else {
     ...
     *
     * @return    Book the result, formatted by the current formatter
     */
